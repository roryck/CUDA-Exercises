...
__constant__ __device__ float A0 = 122.607931777104326f;
__constant__ __device__ float A1 = 214.382388694706425f;
__constant__ __device__ float A2 = 181.928533092181549f;
__constant__ __device__ float A3 = 93.155580458138441f;
__constant__ __device__ float A4 = 30.180142196210589f;
__constant__ __device__ float A5 = 5.912626209773153f;
__constant__ __device__ float A6 = 0.564189583562615f;
__constant__ __device__ float B0 = 122.60793177387535f;
__constant__ __device__ float B1 = 352.730625110963558f;
__constant__ __device__ float B2 = 457.334478783897737f;
__constant__ __device__ float B3 = 348.703917719495792f;
__constant__ __device__ float B4 = 170.354001821091472f;
__constant__ __device__ float B5 = 53.992906912940207f;
__constant__ __device__ float B6 = 10.479857114260399f;
...
__global__ void my_voigt(float *damp_arr, float *offs_arr, float *voigt_value){
  ...
  int idx = (blockDim.x * blockIdx.x + threadIdx.x) + PROBLEM_SIZE*(blockDim.y * blockIdx.y + threadIdx.y);

  /* get local values of damping and offset */
  float damping = damp_arr[idx];
  float offset = offs_arr[idx];

  V =((offset < 0) ? -1 : 1) * offset;
  ...
  division_factor = __fdividef(1.0f, ZZ7_real * ZZ7_real + ZZ7_imag * ZZ7_imag);
